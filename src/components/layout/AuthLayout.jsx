  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <div className="relative hidden w-1/2 overflow-hidden bg-[linear-gradient(150deg,#0A1E3F_0%,#12356B_55%,#1E5AF0_120%)] lg:flex">
        ...
        {/* Hoạ tiết kệ kho mờ */}
        <div className="pointer-events-none absolute inset-x-12 top-1/3 flex flex-col gap-4 opacity-[0.05]">
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className="h-8 rounded bg-white" />
          ))}
        </div>
        ...
        <div className="relative z-10 flex flex-1 flex-col justify-between p-12 text-white">
          ...
          <div className="max-w-md">
            ...
            <ul className="mt-8 space-y-4">
              {HIGHLIGHTS.map((text) => (
                <li key={text} className="flex items-center gap-3 text-[#c7d6f5]">
          ...
          <p className="text-sm text-[#8fa8d8]">
